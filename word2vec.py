...
def naiveSoftmaxLossAndGradient(
    ...
    probas = softmax(np.dot(outsideVectors, centerWordVec)) # shape (num words in vocab, )
    loss = -np.log(probas[outsideWordIdx])                  # shape (1, )
    gradCenterVec = np.dot(outsideVectors.T, probas) - outsideVectors[outsideWordIdx] # shape (word vector length, )
    gradOutsideVecs = centerWordVec * probas.reshape(-1, 1) # shape (num words in vocab, word vector length)
    gradOutsideVecs[outsideWordIdx] -= centerWordVec # this is true for w = o
...
def negSamplingLossAndGradient(
    ...
    loss = -np.log(sigmoid(np.dot(outsideVectors[outsideWordIdx], centerWordVec))) - np.sum(np.log(sigmoid(-np.dot(outsideVectors[negSampleWordIndices], centerWordVec))))
    gradCenterVec = (sigmoid(np.dot(outsideVectors[outsideWordIdx], centerWordVec)) - 1) * outsideVectors[outsideWordIdx] + np.sum((1 - sigmoid(-np.dot(outsideVectors[negSampleWordIndices], centerWordVec)))[:, None] * outsideVectors[negSampleWordIndices], axis=0)
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] = (sigmoid(np.dot(outsideVectors[outsideWordIdx], centerWordVec)) - 1) * centerWordVec
    for i in range(K):
        gradOutsideVecs[negSampleWordIndices[i]] += (1 - sigmoid(-np.dot(outsideVectors[negSampleWordIndices[i]], centerWordVec))) * centerWordVec
